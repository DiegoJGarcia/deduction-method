...
const Form: FC<TypeForm> = ({ submitLabel = ' Aceptar', onSubmit, inputs = [], bottom }) => {
	...
	const handleChange = (name: string, value: any, check?: TypeCheck, label?: string) => {
				...
				{inputs &&
					inputs.map((inp, index) => (
						<Input
							...
							onChange={(e: any) =>
								handleChange(e.target.name, e.target.value, inp.check, inp.label)
							}
							onBlur={() => inp.check && validate(inp.name, inp.check, inp.label, values[inp.name])}
							type={inp.type}
							label={inp.label}
